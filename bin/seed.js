...
books.forEach((oneBook) => {
  Author.create(oneBook.author)
    ...
    .then((authorDetails) => {
      console.log(`Created AUTHOR ${authorDetails.name}`)
      // change "author" to the ObjectId
      oneBook.author = authorDetails._id;
      // return the promise object for BOOK create
      return Book.create(oneBook);
    })
    // BOOK success callback
    .then(() => {
      console.log(`Created BOOK ${oneBook.title}`)
    })
    // AUTHOR & BOOK error callback
    .catch((err) => {
      console.log(`Error creating BOOK ${oneBook.title} or AUTHOR 💩`);
    });
});
